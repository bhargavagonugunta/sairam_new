...
function PaymentMode() {
  ...
  const { totalAmount, totalQuantity, deliveryCharge, total } = useSelector(
  ...
  const handleOverlayClick = (event: React.MouseEvent) => {
    // @ts-ignore
    if (!cardDetailsRef?.current?.contains(event.target as Node)) {
      setShowCardDetails(false);
    }
    // @ts-ignore
    if (!addAddressRef?.current?.contains(event.target as Node)) {
      setShowAddAddress(false);
    }
  };
        ...
        <div className="flex flex-col gap-8 ">
          ...
          <div className="flex flex-col md:flex lg:flex-row gap-10  ">
            <div className="flex flex-col w-3/6 text-left gap-2 md:gap-4 md:order-2">
              ...
              <p className="text-lg font-normal w-[300px]">
                Plot No: 60, Road NO: 5 , Ayyappa Society , Madhapur,Hyderabad -
            ...
            <form className="flex flex-col gap-10">
              <div className="flex flex-col gap-4 md:gap-0">
                <div className="flex gap-2 ">
                  ...
                  <label
                    htmlFor="card"
                    className="text-lg font-medium border border-slate-800 px-4 rounded-lg"
                  >
                    {" "}
                    Credit / Debit card{" "}
                  </label>
                ...
                <div className="flex-col gap-6 pl-4">
                  <div className=" flex items-center gap-4">
                    ...
                    <button
                      type="submit"
                      className="h-[35px] md:h-[50px] bg-[#f7f7f7] px-4 "
                    >
                      See More
                    </button>
                  </div>
                  <div className="flex gap-4 pl-2">
                    ...
                    <a
                      ...
                      className="flex justify-center items-center text-[#158ca6]"
                    >
                      Enter Card Details
                    </a>
                    {showCardDetails && (
                ...
                <div className="flex flex-col gap-2 pl-4">
                  ...
                  <div className="space-x-4">
                    <input
                      ...
                      className="text-black rounded-full p-2 bg-[#e8e8e8] outline-none"
                    />
                    <button
                      type="submit"
                      className="bg-[#fca120] border border-slate-700 rounded-lg px-4"
                    >
                      Verify
                    </button>
                  </div>
